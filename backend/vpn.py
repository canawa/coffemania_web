...
Breaking changes from v2 (see https://f.docs.rw/t/topic/354):
- users are identified by numeric ``id`` (``uuid`` removed);
- ``GET /api/users/by-telegram-id/...`` removed → ``GET /api/users/stream?telegramId=``;
- ``GET /api/hwid/devices/{userId}`` (was uuid /devices/get/...);
- ``PATCH /api/users`` identifies by ``id`` (or ``username``);
- renew via ``POST /api/users/{userId}/actions/extend``.
"""
...
def _looks_like_user(obj: dict) -> bool:
    return any(obj.get(field) not in (None, "") for field in _USER_MARKERS)
...
def _unwrap_remnawave_user(data: Any) -> dict | None:
    if isinstance(data, list):
        for item in data:
            if isinstance(item, dict) and _looks_like_user(item):
                return item
        return None
        ...
        if isinstance(nested, dict):
            ...
            if isinstance(users, list):
                for item in users:
                    if isinstance(item, dict) and _looks_like_user(item):
                        return item
            if _looks_like_user(nested):
                return nested

    if _looks_like_user(data):
        return data
...
def _extract_user_id(user: dict | None) -> int | None:
    ...
    for key in ("id", "userId", "user_id"):
        value = _coerce_int(user.get(key))
...
def _build_create_user_payload(
    username: str,
    expire_at_iso: str,
    email: str | None = None,
...
def _request_json_sync(
    ...
    try:
        ...
        if response.status_code in (204, 202) or not response.content:
...
async def fetch_user_by_id(user_id: int, *, quiet_not_found: bool = False) -> dict | None:
    data, status, error_text = await _request_json("GET", f"/api/users/{user_id}")
    if status >= 400:
        if not (quiet_not_found and status == 404):
            print(f"remnawave fetch user by id failed: {status} | {error_text[:300]}")
        return None
    return _unwrap_remnawave_user(data)


async def fetch_user_by_telegram_id(telegram_id: int, *, quiet_not_found: bool = False) -> dict | None:
    """v3: by-telegram-id removed — use users/stream filter."""
    preferred = remnawave_telegram_username(telegram_id)
    data, status, error_text = await _request_json(
        "GET",
        "/api/users/stream",
        query={"telegramId": telegram_id, "size": 100},
    )
    if status >= 400:
        if not (quiet_not_found and status == 404):
            print(f"remnawave fetch by telegram id (stream) failed: {status} | {error_text[:300]}")
        return None

    users: list[dict] = []
    nested = _unwrap_remnawave_response(data) if isinstance(data, dict) else None
    if isinstance(nested, dict):
        raw_users = nested.get("users")
        if isinstance(raw_users, list):
            users = [item for item in raw_users if isinstance(item, dict)]
    if not users:
        single = _unwrap_remnawave_user(data)
        if single:
            users = [single]

    if not users:
        if not quiet_not_found:
            print(f"remnawave stream returned no users for tg_id={telegram_id}")
        return None

    for user in users:
        if user.get("username") == preferred:
            return user
    return users[0]
...
async def fetch_telegram_bot_subscription(telegram_id: int) -> dict | None:
    ...
    if user:
        ...
        print(f"[remnawave] user {username} not found, api={api_base}")

    tg_user = await fetch_user_by_telegram_id(telegram_id, quiet_not_found=True)
    if tg_user:
        subscription = remnawave_user_to_subscription(tg_user)
        if subscription:
            subscription["username"] = (
                subscription.get("username") or tg_user.get("username") or username
            )
            return subscription
    ...
    unwrapped = _unwrap_remnawave_user(data) or _unwrap_remnawave_response(data)
    if not unwrapped:
...
async def update_remnawave_user(
    *,
    ...
    if user_id is None and not (isinstance(username, str) and username.strip()):
        return None, 400, "Remnawave update requires id or username"
    ...
    if expire_at_iso is not None:
        payload["expireAt"] = expire_at_iso
    ...
    updated = _unwrap_remnawave_user(updated_raw) or (
        _unwrap_remnawave_response(updated_raw) if isinstance(updated_raw, dict) else None
    )
    return updated, update_status, update_error_text


async def extend_remnawave_user(user_id: int, days: int) -> tuple[dict | None, int, str]:
    if days < 1:
        return None, 400, "days must be >= 1"
    data, status, error_text = await _request_json(
        "POST",
    ...
    if status >= 400:
        return None, status, error_text
    updated = _unwrap_remnawave_user(data) or (
        _unwrap_remnawave_response(data) if isinstance(data, dict) else None
    )
    return updated, status, error_text
...
async def attach_telegram_id_to_remnawave_user(
    ...
    user_id = _extract_user_id(rw_user)
    username = rw_user.get("username")
    if user_id is None and not (isinstance(username, str) and username.strip()):
        return False

    _, update_status, update_error_text = await update_remnawave_user(
        user_id=user_id,
        username=None if user_id is not None else str(username).strip(),
        email=email or rw_user.get("email"),
        telegram_id=telegram_id,
    )
    if update_status >= 400:
        print(f"remnawave attach telegramId failed: {update_status} | {update_error_text}")
        return False
    return True
...
async def generate_vpn_key(
    ...
    if not REMNAWAVE_INTERNAL_SQUAD_ID:
        return {
            "error": (
                "Не задан REMNAWAVE_INTERNAL_SQUAD_ID. "
                "Без internal squad Remnawave может возвращать access error."
            )
        }
    username = remnawave_username(user_id)

    existing = await fetch_user_by_username(username, quiet_not_found=True)
    if existing:
    ...
    expire_at = _utc_now() + timedelta(days=duration_days)
    payload = _build_create_user_payload(
        username=username,
        expire_at_iso=_to_iso(expire_at),
        email=email,
        telegram_id=telegram_id,
    )
    ...
    created = _unwrap_remnawave_user(created_raw) or (
        _unwrap_remnawave_response(created_raw) if isinstance(created_raw, dict) else None
    )
...
async def renew_vpn_key(
    ...
    remnawave_id = _extract_user_id(fetched)
    if remnawave_id is None:
        return {"error": f"У пользователя Remnawave нет numeric id: {username}"}

    updated, extend_status, extend_error = await extend_remnawave_user(
        remnawave_id,
        duration_days,
    )
    if extend_status >= 400 or not updated:
        print(f"remnawave extend failed: {extend_status} | {extend_error}")
        return {"error": f"Remnawave renew failed ({extend_status}): {extend_error}"}

    # Keep profile fields in sync without touching expireAt (extend already did that).
    if email is not None or telegram_id is not None:
        patched, patch_status, patch_error = await update_remnawave_user(
        ...
        if patch_status >= 400:
            print(f"remnawave renew patch failed: {patch_status} | {patch_error}")
        elif patched:
            updated = patched

    sub = _extract_subscription_url(updated) or _extract_subscription_url(fetched)
    expire_iso = updated.get("expireAt") or updated.get("expire_at")
    expire_ts: int | None = None
    if isinstance(expire_iso, str):
        try:
            expire_ts = int(
                datetime.fromisoformat(expire_iso.replace("Z", "+00:00")).timestamp()
            )
        except Exception:
            expire_ts = None
    if expire_ts is None:
        expire_ts = int((_utc_now() + timedelta(days=duration_days)).timestamp())
...
async def fetch_user_hwid_devices(user_id: int) -> tuple[int, list[dict]]:
    data, status, error_text = await _request_json(
        "GET",
        f"/api/hwid/devices/{user_id}",
    )
    if status >= 400:
        print(
            f"remnawave fetch hwid devices failed for userId={user_id}: "
            f"{status} | {error_text[:300]}",
        )
        return 0, []
    ...
    if used == 0 and isinstance(data, dict):
        print(
            f"[remnawave] hwid devices parsed as 0 for userId={user_id}, "
...
async def get_account_device_stats(
    ...
    remnawave_id = _extract_user_id(rw_user)
    devices_used = 0
    if remnawave_id is not None:
        devices_used, _ = await fetch_user_hwid_devices(remnawave_id)
    else:
        print(
            f"[remnawave] no numeric id for user {rw_user.get('username')}, "
...
async def add_hwid_device_slots(rw_user: dict, amount: int) -> tuple[bool, int]:
    ...
    _, status, error_text = await update_remnawave_user(
        user_id=user_id,
        username=None if user_id is not None else str(username).strip(),
